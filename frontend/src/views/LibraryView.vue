...
import { BarChart3, FileText, FileUp, ImagePlus, Link2, LogIn, MessageCircle, Plus, Search, Send, Trash2, X } from '@lucide/vue'
...
function selectType(type?: CommunityPostType) { activeType.value = type; void load(1) }
function selectTag(tagId: number) { activeTag.value = tagId; void load(1) }
function clearTag() { activeTag.value = undefined; void load(1) }
function resetFilters() { activeTag.value = undefined; activeType.value = undefined; keyword.value = ''; void load(1) }
...
async function load(next = 1) {
  error.value = ''
  try { page.value = await fetchCommunityPosts({ keyword: keyword.value.trim(), tagId: activeTag.value, postType: activeType.value, page: next, size: 12 }) }
...
<template>
  <section class="share-library content-width">
    <header class="share-heading">
      ...
      <button v-if="authStore.state.user" class="primary-button" type="button" @click="showComposer = !showComposer"><Plus :size="18" />发布分享</button>
      <RouterLink v-else class="primary-button" to="/login"><LogIn :size="18" />登录后发布</RouterLink>
    </header>
    <Teleport to="body">
