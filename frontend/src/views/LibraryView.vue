...
<template>
  <section class="share-library content-width">
    ...
    <div class="share-layout">
      <aside class="share-filter"><label class="library-search"><Search :size="18" /><input v-model="keyword" placeholder="搜索分享" @keyup.enter="load(1)" /></label><button :class="{ active: !activeTag && !activeType }" type="button" @click="activeTag = undefined; activeType = undefined; load(1)">全部分享 <small>{{ page.total }}</small></button><div class="share-filter__label">卡片类型</div><button v-for="(_, type) in typeMeta" :key="type" :class="{ active: activeType === type }" type="button" @click="activeType = type as CommunityPostType; load(1)">{{ typeMeta[type as CommunityPostType] }}</button><div class="share-filter__label">知识标签</div><button v-for="tag in flatTags" :key="tag.id" :class="{ active: activeTag === tag.id }" type="button" @click="activeTag = tag.id; load(1)">{{ '　'.repeat(tag.level - 1) }}{{ tag.name }}</button></aside>
