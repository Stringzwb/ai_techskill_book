...
<template>
  <section class="document-reader-page">
    <div v-if="loading" class="reader-state">
      <FileText :size="25" />正在读取文档…
    </div>
    <div
      v-else-if="errorMessage || !document"
      class="reader-state reader-state--error"
    >
      <strong>{{ errorMessage || "文档不存在" }}</strong>
      <RouterLink class="secondary-button" to="/documents"
        ><ArrowLeft :size="16" />返回文档库</RouterLink
      >
    </div>
    <template v-else>
      <header class="reader-header">
        <div class="reader-header__inner">
          <RouterLink class="reader-back" to="/documents"
            ><ArrowLeft :size="16" />文档库</RouterLink
          >
          ...
          <div class="reader-meta">
            <span
              ><Clock3 :size="15" />约 {{ document.readingMinutes }} 分钟</span
            >
            <span
            ...
            <ShareTrigger
              ...
              :path="`/share/documents/${document.id}`"
            />
          </div>
        </div>
      </header>
