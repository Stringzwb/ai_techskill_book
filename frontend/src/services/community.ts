...
export function fetchCommunityPosts(params: { keyword?: string; tagId?: number; postType?: CommunityPost['postType']; page?: number; size?: number } = {}) { const q = new URLSearchParams({ page: String(params.page ?? 1), size: String(params.size ?? 12) }); if(params.keyword)q.set('keyword',params.keyword);if(params.tagId)q.set('tagId',String(params.tagId));if(params.postType)q.set('postType',params.postType);return apiRequest<CommunityPostPage>(`/api/community/posts?${q}`) }
